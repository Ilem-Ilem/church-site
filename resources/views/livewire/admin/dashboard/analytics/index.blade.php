...
use App\Models\{Events, EventForm, BeliversAcademy, StudentClasses, Partnership, PrayerRequest, Sermon, Transport, Accounts, Chapter};
...
new #[Layout('components.layouts.admin')] class extends Component {
    ...
    public function loadAnalytics()
    {
        ...
        $this->totalStudents = StudentClasses::when($this->chapterId, fn($q) => $q->where('chapter_id', $this->chapterId))
            ->where('created_at', '>=', $startDate)->distinct('account_id')->count();
        ...
        $this->totalSermons = Sermon::when($this->chapterId, fn($q) => $q->where('chapter_id', $this->chapterId))
            ->where('created_at', '>=', $startDate)->count();
    ...
    private function loadAcademyAnalytics($startDate)
    {
        ...
        $enrollmentTrend = StudentClasses::when($this->chapterId, fn($q) => $q->where('chapter_id', $this->chapterId))
            ->where('created_at', '>=', $startDate)
            ->select(DB::raw('DATE(created_at) as date'), DB::raw('COUNT(DISTINCT account_id) as count'))
            ->groupBy('date')
            ->orderBy('date')
            ->get();
        ...
        $totalClasses = StudentClasses::when($this->chapterId, fn($q) => $q->where('chapter_id', $this->chapterId))->count();
        $completedClasses = StudentClasses::when($this->chapterId, fn($q) => $q->where('chapter_id', $this->chapterId))
            ->where('is_completed', true)->count();
        $this->classCompletionRate = $totalClasses > 0 ? round(($completedClasses / $totalClasses) * 100, 1) : 0;

        // Top performing classes
        $this->topPerformingClasses = StudentClasses::when($this->chapterId, fn($q) => $q->where('chapter_id', $this->chapterId))
            ->select('class_id', DB::raw('COUNT(*) as enrollments'))
            ->with('class')
            ->groupBy('class_id')
            ...
            ->get()
            ->map(fn($sc) => [
                'class_name' => $sc->class->title ?? 'Unknown',
    ...
    {
        $this->toast()->info('Export', "Preparing {$format} export. Implementation coming soon...")->send();

        // TODO: Implement actual export functionality
        // Will add PDF/Excel/CSV export in next step
    }
}; ?>
