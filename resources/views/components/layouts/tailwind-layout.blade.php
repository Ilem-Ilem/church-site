...
<body class="site-shell" style="--mobile-nav-height: 5.25rem;">
    <x-flash />
    ...
    <header class="sticky top-0 z-50 border-b border-blue-100/80 bg-white/95 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-white/85">
        <nav class="mx-auto flex min-h-[4.5rem] w-full max-w-7xl items-center justify-between px-4 py-3.5 sm:px-6 lg:px-8">
            <a href="{{ route('home') }}" wire:navigate class="flex items-center gap-3">
                <img src="{{ asset('Img/doxa.PNG') }}" alt="Doxa Commission Global" class="h-10 w-10 rounded-full object-cover ring-1 ring-blue-100 sm:h-11 sm:w-11">
                <span class="hidden text-sm font-semibold tracking-wide text-blue-700 sm:block">Doxa Commission Global</span>
            </a>
            ...
            <div class="hidden items-center gap-3 lg:flex">
                @auth
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="rounded-full border border-blue-200 px-4 py-2 text-sm font-medium text-blue-700 transition hover:border-blue-300 hover:text-blue-800">
                            Logout
                        </button>
                    </form>
                @else
                    <a href="{{ route('home.login') }}" wire:navigate class="rounded-full bg-blue-600 px-5 py-2 text-sm font-semibold text-white transition hover:bg-blue-700">
                        Connect With Us
                    </a>
                @endauth
            </div>
            ...
            <div class="flex items-center gap-3 lg:hidden">
                @auth
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="rounded-full border border-blue-200 px-3 py-1.5 text-xs font-medium text-blue-700">Logout</button>
                    </form>
                @else
                    <a href="{{ route('home.login') }}" wire:navigate class="rounded-full bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white">Login</a>
                @endauth
            </div>
        </nav>
    </header>
    ...
    <footer class="mt-14 border-t border-blue-100 bg-blue-700 text-blue-50">
        <div class="mx-auto grid w-full max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-4 lg:px-8">
            <div class="space-y-3">
                <h3 class="text-base font-semibold text-white">Doxa Commission Global</h3>
                <p class="text-sm text-blue-100">Bringing nations into God's glory worldwide.</p>
                <p class="text-sm text-blue-100">129 Goldie, Adjacent Amika Utuk, Calabar, Cross River State, Nigeria.</p>
                <a href="tel:+2341234567890" class="block text-sm text-blue-100 transition hover:text-white">+234 1234567890</a>
                <a href="mailto:info@doxachurch.org" class="block text-sm text-blue-100 transition hover:text-white">info@doxachurch.org</a>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-wrap gap-2 text-sm">
                    <a href="https://www.facebook.com/DoxaCommissionGlobal/" target="_blank" rel="noopener noreferrer" class="rounded-full border border-blue-300/60 px-3 py-1.5 text-blue-50 transition hover:border-white hover:text-white">Facebook Live</a>
                    <a href="https://x.com" target="_blank" rel="noopener noreferrer" class="rounded-full border border-blue-300/60 px-3 py-1.5 text-blue-50 transition hover:border-white hover:text-white">X / Twitter</a>
                    <a href="https://www.youtube.com/channel/UCZtReUAxK3S6qBKnV5G6PTw" target="_blank" rel="noopener noreferrer" class="rounded-full border border-blue-300/60 px-3 py-1.5 text-blue-50 transition hover:border-white hover:text-white">YouTube</a>
                </div>
                <div class="flex items-center gap-4 text-lg">
                    <a href="https://www.facebook.com/DoxaCommissionGlobal/" target="_blank" rel="noopener noreferrer" aria-label="Facebook" class="text-blue-100 transition hover:text-white"><i class="fab fa-facebook-f"></i></a>
                    <a href="https://www.instagram.com/doxa_commission/?hl=en" target="_blank" rel="noopener noreferrer" aria-label="Instagram" class="text-blue-100 transition hover:text-white"><i class="fab fa-instagram"></i></a>
                    <a href="https://www.youtube.com/channel/UCZtReUAxK3S6qBKnV5G6PTw" target="_blank" rel="noopener noreferrer" aria-label="YouTube" class="text-blue-100 transition hover:text-white"><i class="fab fa-youtube"></i></a>
                    <a href="https://www.tiktok.com/discover/doxa-commission-global" target="_blank" rel="noopener noreferrer" aria-label="TikTok" class="text-blue-100 transition hover:text-white"><i class="fab fa-tiktok"></i></a>
                </div>
            </div>
        </div>

        <div class="border-t border-blue-600 px-4 py-5 text-center text-xs text-blue-100">
            &copy; <span id="current-year"></span> Doxa Commission Global. All Rights Reserved.
        </div>
    </footer>
    ...
    <nav class="mobile-floating-nav lg:hidden" aria-label="Primary">
        @foreach($primaryNav as $item)
            ...
            >
                <i class="fas {{ $item['icon'] }} text-base" aria-hidden="true"></i>
